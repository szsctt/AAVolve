...
- `minimap2_params` (optional): Additional parameters to pass to minimap2 during alignment. If a preset (`-x`), end bonus (`--end-bonus`) or mismatch penatly (`-B`) are not specified, the defaults `-x map-hifi`, `-B 1.5` and `--end-bonus 5` will be used. (default: --x map-hifi -B 1.5 --end-bonus 5)

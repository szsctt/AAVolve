...
- `non_parental_group` (optional): When `include_non_parental` is `True`, set this to control which samples share non-parental variants. Samples only share variants within matching `parent_file`/`reference_file` groups, and within the same `non_parental_group`. Use this to run the same underlying reads with different parameters (e.g. different `non_parental_freq` thresholds) without cross-contaminating the non-parental variant sets between runs.
- `require_end_to_end_alignment` (optional): If `True`, require reads to align end-to-end to the reference when extracting variants (i.e. full reference coverage), even when multiple parents are provided. This can reduce the number of reads retained; leave as `False` to only require coverage from the first to last parental variant. When `include_non_parental` is enabled and high-frequency non-parental variants fall outside the parental window, AAVolve will emit a warning in the sample report. (default: False)
- `group_vars` (optional): During assignment of parents, group variants and assign parent with lowest hamming distance to read.  If False, variants are considered individually (default: False)
- Note: `group_vars` and `include_non_parental` cannot both be `True`, because grouped parent-assignment assumes each variant group can be explained by parental haplotypes; including non-parental variants in grouping would produce ambiguous/NA-heavy assignments.
